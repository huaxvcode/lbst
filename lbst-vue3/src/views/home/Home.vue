...
const updateHeadImgUrl = async () => {
  ...
  await getUserInfo(userId).then(resp => resp.data).then(data => {
    if (data.data && data.data.headImg) {
      const headImgName = data.data.headImg;
      getHeadImgUrl(headImgName).then(url => {
...
<template>
  <div class="home-view">
    <el-container style="height: 100%;">
        ...
        <el-scrollbar>
          <el-collapse v-model="activeNames" @change="handleChange">
            <template v-for="(item, index) in asideTag" :key="index">
              <el-collapse-item :name="index" v-if="item.role >= role && item.children && item.children.length > 0">
                <template #title>
                ...
                <template v-for="(iitem, index) in item.children" :key="index">
                  <div class="item over-mouse-hand" v-if="iitem.role && iitem.role >= role"
              ...
              <div class="empty-collapse over-mouse-hand" v-else-if="item.role >= role"
      ...
      <el-container>
        <el-header style="height: 48px; padding-left: 12px;">
          <div class="header">
            <div @click="asideWidth = (asideWidth + 250) % 500" class="header-title">
            ...
            <el-dropdown class="head-icon">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item @click="logoutClick">注销登录</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </div>
